	...
	return await new Promise((resolve, reject) => {
		...
		// tables/table1.xml
		{
			...
			if (chartObject.opts._type === CHART_TYPE.BUBBLE || chartObject.opts._type === CHART_TYPE.BUBBLE3D) {
				...
				strTableXml += `<table xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" id="1" name="Table1" displayName="Table1" ref="A1:${getExcelColName(data.length)}${
...
export function makeXmlCharts(rel: ISlideRelChart): string {
	...
	// STEP 1: Create chart
	{
		...
		if (rel.opts._type === CHART_TYPE.BAR3D) {
			strXml += `<c:view3D><c:rotX val="${rel.opts.v3DRotX}"/><c:rotY val="${rel.opts.v3DRotY}"/><c:rAngAx val="${!rel.opts.v3DRAngAx ? 0 : 1}"/><c:perspective val="${
				rel.opts.v3DPerspective
			}"/></c:view3D>`
		}
            ...
            */
			...
			data.forEach((obj) => {
				...
				if (chartType === CHART_TYPE.LINE || chartType === CHART_TYPE.RADAR) {
					...
					strXml += `<a:ln w="${valToPts(opts.dataBorder.pt)}" cap="${createLineCap(opts.lineCap)}"><a:solidFill>${createColorElement(
				...
				if (chartType === CHART_TYPE.LINE || chartType === CHART_TYPE.RADAR) {
					...
					strXml += `    <a:ln w="${opts.lineDataSymbolLineSize}" cap="flat"><a:solidFill>${createColorElement(
						opts.lineDataSymbolLineColor || seriesColor
					)}</a:solidFill><a:prstDash val="solid"/><a:round/></a:ln>`
					strXml += '    <a:effectLst/>'
					strXml += '  </c:spPr>'
					strXml += '</c:marker>'
            ...
            */
			...
			data.filter((_obj, idx) => idx > 0).forEach((obj, idx) => {
				...
				// 'c:marker' tag: `lineDataSymbol`
				{
					...
					strXml += `<a:ln w="${opts.lineDataSymbolLineSize}" cap="flat"><a:solidFill>${createColorElement(
						opts.lineDataSymbolLineColor || opts.chartColors[colorIndex % opts.chartColors.length]
					)}</a:solidFill><a:prstDash val="solid"/><a:round/></a:ln>`
					strXml += '<a:effectLst/>'
					strXml += '</c:spPr>'
					strXml += '</c:marker>'
...
function makeCatAxis(opts: IChartOptsLib, axisId: string, valAxisId: string): string {
	...
	strXml += '    <a:p>'
	strXml += '    <a:pPr>'
	strXml += `      <a:defRPr sz="${Math.round((opts.catAxisLabelFontSize || DEF_FONT_SIZE) * 100)}" b="${opts.catAxisLabelFontBold ? 1 : 0}" i="${
...
function makeValAxis(opts: IChartOptsLib, valAxisId: string): string {
	...
	strXml += '  <a:p>'
	strXml += '    <a:pPr>'
	strXml += `      <a:defRPr sz="${Math.round((opts.valAxisLabelFontSize || DEF_FONT_SIZE) * 100)}" b="${opts.valAxisLabelFontBold ? 1 : 0}" i="${
...
function makeSerAxis(opts: IChartOptsLib, axisId: string, valAxisId: string): string {
	...
	strXml += '    <a:p>'
	strXml += '    <a:pPr>'
	strXml += `    <a:defRPr sz="${Math.round((opts.serAxisLabelFontSize || DEF_FONT_SIZE) * 100)}" b="${opts.serAxisLabelFontBold ? '1' : '0'}" i="${
